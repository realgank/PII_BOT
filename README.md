...
#### `/info`
- **Что делает:** показывает подробную памятку на английском языке о работе `/addpos`: параметры, использование значений по умолчанию, правила владения, подбор планет, а также как считаются лимиты слотов/буров и почему итоговое число планет может быть меньше заявленного.

#### Пошаговая инструкция по `/addpos`
1. Убедитесь, что для сервера настроены значения по умолчанию через `/posdefaults setguild` (если не зададите `slots` и `drills` при вызове, будут использованы эти цифры).
2. Вызовите `/addpos` в нужном сервере Discord и укажите минимум два обязательных параметра: `name` (произвольное название вашего POS) и `system` (выберите систему из автодополнения). Параметры `slots` и `drills` можно оставить пустыми, чтобы подтянулись значения по умолчанию.
3. После отправки команды дождитесь ответа бота: он создаст POS (или обновит существующий) и сразу распределит планеты и буры по текущим целям сервера.
4. Если POS уже принадлежит вам, обновление пройдёт без подтверждений. Если POS занят другим пользователем, изменить его сможете только вы как владелец или администратор сервера.
5. При необходимости уточните детали через `/info` — команда вернёт справку о правилах распределения планет, лимитах слотов/буров и особенностях расчёта.
