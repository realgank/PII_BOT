...
from pii_bot.localization import (
    DEFAULT_LANGUAGE,
    SUPPORTED_LANGUAGES,
    normalize_language,
...
def _get_user_language_from_db(user_id: int) -> Optional[str]:
...
def get_user_language(user_id: Optional[int]) -> str:
    ...
    user_id = int(user_id)
    lang = _user_language_cache.get(user_id)
    if lang:
        return lang
    lang = _get_user_language_from_db(user_id) or DEFAULT_LANGUAGE
    _user_language_cache[user_id] = lang
    return lang


def set_user_language_preference(user_id: int, language: str) -> None:
    ...
    try:
        ...
        cur.execute(
            ...
            """,
            (int(user_id), language, now_utc_iso()),
        )
        conn.commit()
    finally:
        conn.close()
    _user_language_cache[int(user_id)] = language


def get_interaction_language(interaction: Optional[discord.Interaction]) -> str:
    if interaction is None:
        return DEFAULT_LANGUAGE
    user = getattr(interaction, "user", None)
    language = get_user_language(getattr(user, "id", None))
    token = getattr(interaction, "token", None)
    if token:
        _token_language_cache[token] = language
    return language


def get_token_language(token: Optional[str]) -> str:
    if token and token in _token_language_cache:
        return _token_language_cache[token]
    return DEFAULT_LANGUAGE


async def translate_for_interaction(interaction: Optional[discord.Interaction], text: Optional[str]) -> Optional[str]:
    language = get_interaction_language(interaction)
    return await translate_text(text, language)


async def translate_for_user(user: Optional[Union[discord.User, discord.Member]], text: Optional[str]) -> Optional[str]:
    language = get_user_language(getattr(user, "id", None))
    return await translate_text(text, language)


_original_response_send_message = discord.InteractionResponse.send_message
_original_response_defer = discord.InteractionResponse.defer
_original_webhook_send = discord.webhook.async_.Webhook.send
_original_messageable_send = discord.abc.Messageable.send


async def _localized_response_send_message(self: discord.InteractionResponse, *args, **kwargs):
    interaction = getattr(self, "_parent", None)
    language = get_interaction_language(interaction)

    async def _translate(value: Optional[str]) -> Optional[str]:
        if value is None or value is discord.utils.MISSING:
            return value
        return await translate_text(value, language)

    if args:
        translated = await _translate(args[0])
        args = (translated,) + args[1:]
    if "content" in kwargs:
        kwargs["content"] = await _translate(kwargs.get("content"))
    return await _original_response_send_message(self, *args, **kwargs)


async def _localized_response_defer(self: discord.InteractionResponse, *args, **kwargs):
    interaction = getattr(self, "_parent", None)
    get_interaction_language(interaction)
    return await _original_response_defer(self, *args, **kwargs)


async def _localized_webhook_send(self: discord.webhook.async_.Webhook, *args, **kwargs):
    language = get_token_language(getattr(self, "token", None))
...
async def _localized_messageable_send(self: discord.abc.Messageable, *args, **kwargs):
    target_user: Optional[Union[discord.User, discord.Member]] = None
    if isinstance(self, (discord.User, discord.Member)):
        target_user = self
    elif isinstance(self, discord.DMChannel):
        target_user = getattr(self, "recipient", None)
    ...
    async def _translate(value: Optional[str]) -> Optional[str]:
        ...
        if target_user is None:
            return value
        return await translate_for_user(target_user, value)
...
discord.InteractionResponse.send_message = _localized_response_send_message  # type: ignore[assignment]
discord.InteractionResponse.defer = _localized_response_defer  # type: ignore[assignment]
discord.webhook.async_.Webhook.send = _localized_webhook_send  # type: ignore[assignment]
discord.abc.Messageable.send = _localized_messageable_send  # type: ignore[assignment]
...
@tree.command(name="language", description="Switch bot language / Сменить язык бота")
@app_commands.describe(language="Язык: ru или en")
async def set_language_command(
    interaction: discord.Interaction,
    language: Optional[str] = None,
):
    await interaction.response.defer(ephemeral=True)
    user = getattr(interaction, "user", None)
    current_language = get_user_language(getattr(user, "id", None))
    available = ", ".join(
        f"{info['emoji']} {code.upper()}" for code, info in SUPPORTED_LANGUAGES.items()
    )
        ...
        message = "\n".join(
            [
                translate_for_language(
                    current_language,
                    "language_current",
                    language_emoji=SUPPORTED_LANGUAGES[current_language]["emoji"],
                    language_name=SUPPORTED_LANGUAGES[current_language]["name"],
                ),
                translate_for_language(current_language, "language_options", languages=available),
                translate_for_language(current_language, "language_usage"),
            ]
        )
    ...
    if requested not in SUPPORTED_LANGUAGES:
        message = "\n".join(
            [
                translate_for_language(current_language, "language_invalid"),
                translate_for_language(current_language, "language_options", languages=available),
            ]
        )
        await interaction.followup.send(message, ephemeral=True)
    ...
    set_user_language_preference(int(user.id), requested)
    token = getattr(interaction, "token", None)
    if token:
        _token_language_cache[token] = requested
    info = SUPPORTED_LANGUAGES[requested]
    await interaction.followup.send(
        translate_for_language(
            requested,
            "language_changed",
